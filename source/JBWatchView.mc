...
class JBWatchView extends Ui.WatchFace {
  ...
  function onUpdate(dc) {   
    ...
    if (JBWatchApp.sleepStart > JBWatchApp.sleepEnd ) {
      ...
      if (logLevel > 2) {
        System.println("onUpdate end Mem :"+System.getSystemStats().freeMemory); 
      }
  ...
  function monthFace(today,dc) {
    ...
    if (logLevel > 2) {  System.println("month end"); }       
}

// HOURS
function hoursFace(today,dc) {
  if (logLevel > 2) {
    System.println("hours start");
  }
  ...
  function minuteHand(dc) {
    if (logLevel > 2) {
      System.println("minute hand start");
    }
    ...
    for (var i = Math.PI * -1.5; i <Math.PI * 0.5 ; i += Math.PI / 120) {
      if ( m == Math.round(clockTime.hour * 10 + q) ) {
        ...
        dc.setColor(JBWatchApp.faceForegroundColor, JBWatchApp.faceBackgroundColor);
        if (logLevel > 2) {
          System.println("minute hand exit");
        }
        break;
      }
        m++; 
  ...
  function showMessage(dc) {
    if (logLevel > 2) {
      System.println("showMessage: " + JBWatchApp.anyMessage); 
      if ( JBWatchApp.anyMessage != null && JBWatchApp.anyMessage.length() > 0  ) {
